...
for name, script in pairs(utils) do
...
local javascript = [[
	...
	let _Create = function(layout, utils) {
...
local function register_for_context(context_name)
	...
	if context[context_name] ~= nil then
		return false
	elseif type(context_name) ~= "string" and context_name:gsub(" ", "") ~= "" then
		return error("invalid context, expected a non-empty string")
	end

	local js = context_name == "" and panorama.loadstring(javascript)() or panorama.loadstring(javascript, context_name)()

	js.create(layout, utils)
	context[context_name] = js
end
register_for_context()

client.set_event_callback("shutdown", function()
	for name, context in pairs(context) do
		context.destroy()
	end
end)
